...
labels_map에 보면 bus는 other-vehicle로 맵핑 되어있지만 METAINFO에는 bus가 존재 ```semantickitti-api```의 [yaml](https://github.com/PRBonn/semantic-kitti-api/blob/master/config/semantic-kitti.yaml) 참고하여 번호 순서에 맞게 MEATAINFO 수정
